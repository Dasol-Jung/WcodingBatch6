...
(function(global, factory) {
	...
	function computeHeightAndMargins(el) {
		...
		return (
			el.getBoundingClientRect().height + parseInt(computed.marginTop, 10) + parseInt(computed.marginBottom, 10)
		);
	}
        ...
        */
		...
		DateComponent.prototype.isValidSegDownEl = function(el) {
			return (
				...
				!this.isInPopover(el)
			); // how to determine if not in a sub-component???
		};
		...
		parse: function(rawEvent, allDayDefault, leftoverProps, dateEnv) {
			...
			if (anyValid) {
				return {
					allDay: allDay,
					duration:
						props.startTime && props.endTime ? subtractDurations(props.endTime, props.startTime) : null,
	...
	function parse(str) {
		...
		if (m) {
			...
			if (isTimeUnspecified) {
				...
				str =
					str.replace(ISO_TZO_RE, function(whole, z, sign, minutes, seconds) {
						if (z) {
							timeZoneOffset = 0;
						} else {
							timeZoneOffset =
								(parseInt(minutes, 10) * 60 + parseInt(seconds || 0, 10)) * (sign === '-' ? -1 : 1);
						}
						return '';
		...
		Toolbar.prototype.updateTitle = function(text) {
			findElements(this.el, 'h2').forEach(function(titleEl) {
				if (!text.includes(',')) {
					...
					titleEl.id = 'calIcon';
					titleEl.innerHTML = `<span class='month'>${text.slice(0, 3)}</span><span class='year'>${text.slice(
						text.length - 4
					)}`;
	...
	var CalendarComponent /** @class */ = (function(_super) {
		...
		CalendarComponent.prototype.computeHeightVars = function() {
			...
			if (typeof contentHeightInput === 'number') {
				...
				this.viewHeight = Math.round(this.contentEl.offsetWidth / Math.max(calendar.opt('aspectRatio'), 0.5));
			}
		};
